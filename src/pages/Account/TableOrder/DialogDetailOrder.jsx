...
function DialogDetailOrder({ isOpen, handleOpen, productId }) {
  ...
  return (
    <>
      {productId !== '' && (
        <Dialog open={isOpen} size="lg" handler={handleOpen} className="overflow-y-auto max-h-[90vh] relative">
          {true && (
                ...
                <div className="w-3/5">
                  <div className="flex justify-between items-center mb-10">
                    <h1 className="font-bold text-2xl">
                      Thông tin đơn hàng{''}
                      <span className="text-[14px] text-[#ff1f1f]">({resultOneOder?.data?.data?.products.length} sản phẩm)</span>
                    </h1>
                    <div className="w-max">
                      <Chip
                        size="sm"
                        variant="ghost"
                        value={
                           resultOneOder?.data?.data?.status === 'paid'
                            ? 'Đã thanh toán'
                            :  resultOneOder?.data?.data?.status === 'pending'
                              ? 'Đang chờ xử lý'
                              : 'Đã hủy'
                        }
                  ...
                  <div className="max-h-[50vh] overflow-y-auto">
                    {!resultOneOder.isFetching &&
                      resultOneOder?.data?.data?.products.map((item, index) => {
