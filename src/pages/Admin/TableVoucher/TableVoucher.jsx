...
import DialogHandleVoucher from './DialogHandleVoucher';
import { useEffect, useState } from 'react';
import { FaPlus } from 'react-icons/fa6';
import { BsTrash } from 'react-icons/bs';
import { AiOutlineEdit } from 'react-icons/ai';
import { useLazyGetAllVouchersQuery, useDeleteVoucherMutation } from '../../../features/Vourcher/voucherApi.service';
import { errorNotify, successNotify } from '../../../components/Toast';

const TABLE_HEAD = ['Mã Coupon', 'Mô tả', 'slug', 'Chức năng'];

function TableVoucher() {
  //   const classes = '';
  const [deleteVoucher, resultdeleteVoucher] = useDeleteVoucherMutation();
  const [page, setPage] = useState(1);
  const [getAllVoucher, resultVoucher] = useLazyGetAllVouchersQuery();
  ...
  const handleAction = () => {
    if (open.type === 'delete') {
      deleteVoucher({
        ...
        .unwrap()
        .then(() => {
          successNotify('Xóa mã giảm giá thành công');
        })
        .catch((err) => {
          errorNotify('Xóa mã giảm giá thất bại');
        });
    } else {
      // updateRole({
      //     id: open.id,
      ...
      <Card className="w-full">
        ...
        <CardBody className="overflow-scroll px-0">
          <table className="mt-4 w-full min-w-max table-auto text-left">
            ...
            <tbody>
              {resultVoucher?.data?.data?.map((voucher, index) => {
                const isLast = index === resultVoucher?.data?.data?.length - 1;
                ...
                return (
                  <tr>
                    ...
                    <Tooltip content="Thay đổi vai trò">
                      ...
                      <Tooltip content="Xóa mã giảm giá">
                        <IconButton
                          variant="text"
                          onClick={() =>
                            setOpen({
        ...
        <CardFooter className="flex items-center justify-between border-t border-blue-gray-50 p-4">
          <Typography variant="small" color="blue-gray" className="font-normal">
            Trang {page} trên {resultVoucher?.data?.totalPages} tổng số {resultVoucher?.data?.totalVouchers}
          </Typography>
          <div className="flex gap-2">
            <Button variant="outlined" size="sm" disabled={page === 1} onClick={() => setPage(page - 1)}>
              Trước
            </Button>
            <Button variant="outlined" size="sm" onClick={() => setPage(page + 1)} disabled={page === resultVoucher?.data?.totalPages}>
              Sau
            </Button>
          </div>
        </CardFooter>
